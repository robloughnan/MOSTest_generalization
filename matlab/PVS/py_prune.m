function [bim_survive] = py_prune(bim, pval_file, stat)
    % bim_survive is a vector the same length as bim with Trues where it survives pruning from plink
    sumstat_py = '~/Programs/python_convert/sumstats.py';
    ...
    sumstat_out = strrep(pval_file, 'mat.mat', [stat, '.sumstats']);
    ...
    py_clump_cmd = sprintf('conda activate py2\npython %s clump --clump-field PVAL --plink plink --sumstats %s --bfile-chr %s --exclude-ranges 6:25119106-33854733 --clump-p1 5E-8 --out %s',...
     sumstat_py, sumstat_out, LD_ref, clump_out);
