function [coef, t, n] = nancorr_col(A, B)
    %NANCORR - Pearson correlation coefficient
    %
    %  coef = NANCORR(A, B) is equivalent to 
    %  coef = corr(A, B, 'rows','pairwise'),
    %  but only computes for pairs of columns
    %  (i.e. diag(coef) and diag(t)) and 
    %  NANCORR works much faster.
    %  
    ...
    xy = dot(A, B);          % sum x_i y_i
    n  = dot(Ap, Bp);        % number of items defined both in x and y
    
    mx = dot(A, Bp) ./ n;    % mean values in x, calculated across items defined both in x and y
    my = dot(Ap, B) ./ n;    % mean values in y, calculated across items defined both in x and y
    
    x2 = dot((A.*A), Bp);    % sum x^2_i, calculated across items defined both in x and y
    y2 = dot(Ap, (B.*B));    % sum y^2_i, calculated across items defined both in x and y
